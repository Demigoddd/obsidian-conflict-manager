...
export class UnifiedDiff {
  static render(container: HTMLElement, mainText: string, conflictText: string): void {
    ...
    if (!rows.some((r) => r.type !== 'unchanged'))
      return void container.createEl('h4', { text: 'Files are identical', cls: 'empty-text' });
    ...
    while (i < parts.length) {
      ...
      if (current.removed) {
        currentText.forEach((t) =>
          rows.push({ type: 'delete', text: t, mainNumber: m++, conflictNumber: null }),
        );
        i++;
        continue;
      }
      ...
      if (current.added) {
        currentText.forEach((t) =>
          rows.push({ type: 'insert', text: t, mainNumber: null, conflictNumber: c++ }),
        );
        i++;
        continue;
      }
      ...
      currentText.forEach((t) =>
        rows.push({ type: 'unchanged', text: t, mainNumber: m++, conflictNumber: c++ }),
      );
      i++;
    }
    ...
    rows.forEach((r, i) => {
      if (r.type !== 'unchanged') {
        for (
          ...
          k <= Math.min(rows.length - 1, i + CONTEXT_LINES);
          k++
        ) {
          show.add(k);
        }
  ...
  private static paintRow(parent: HTMLElement, row: Row): void {
    ...
    el.createSpan({
      cls: 'number',
      text: row.conflictNumber != null ? String(row.conflictNumber) : '',
    });
