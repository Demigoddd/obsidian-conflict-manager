...
// The elements the banner is prepended to, per view type.
// Markdown keeps both mode wrappers mounted and hides the inactive one, so it lists both.
...
export class ConflictManagerNotifier {
  ...
  createConflictBanner(view: EditableFileView, activeFile: TFile, conflictFiles: TFile[]) {
    ...
    // Every mode wrapper gets its own banner, so switching modes keeps it visible
    for (const host of this.getBannerHosts(view)) {
      this.renderBanner(host, view, activeFile, conflictFiles);
    }
  }
  ...
  private getBannerHosts(view: EditableFileView): HTMLElement[] {
    const hosts = (BANNER_HOSTS[view.getViewType()] ?? [])
      .map((selector) => view.containerEl.querySelector<HTMLElement>(selector))
      .filter((host): host is HTMLElement => host !== null);

    return hosts.length > 0 ? hosts : [view.contentEl];
  }
}
